...
class _UserContainerState extends State<UserContainer> {
  final _nController = TextEditingController(text: "João");
  ...
  Widget _buildNameField() {
    return TextFormField(
      ...
      validator: (val) {
        if (val.isEmpty) {
          return "Seu nome não pode estár vazio!";
        }
        return null;
      },
    );
      ...
      child: Container(
        child: TextFormField(
          ...
          decoration: InputDecoration(
            labelText: "Preço",
            suffix: Text("BRL", style: TextStyle(fontSize: 18)),
          ),
          onSaved: (val) =>
          ...
          validator: (val) {
            if (val.isEmpty) {
              return "Digite o preço que você pagou!";
            }
            double value = double.tryParse(val);
            if (value == null || value < 0) {
              return "Isso não é um número válido!";
            }
            return null;
          },
        ),
